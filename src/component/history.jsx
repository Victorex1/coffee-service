    ...
    return ( 
        <div className="pt-20 px-5 relative w-screen flex flex-col items-center">
           ...
           {history &&  <div className=' itembox w-full my-4'>
             {history.map( each => (  
                 <div key={each.id}  className='w-full p-3 bg-gray-300 text-black rounded-2xl flex justify-between items-start my-3'>
                <div className=''>
                  <h4 className='text-xl test-gray-300 font-bold'>{each[0].date}</h4>
                  <p className='text-sm te text-gray-500'>Items: <span>{each[1].length}</span></p>
                  <p className='text-sm text-gray-500'>Time: <span>{each[0].time}</span></p>
